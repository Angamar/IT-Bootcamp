...
--selektovati sve zanrove poredjane po abecednom redu
SELECT DISTINCT zanr FROM filmovi
ORDER BY zanr;

--selektovati sve filmove u obliku "naslov(reziser)" (u jednoj koloni
SELECT CONCAT (naslov, ' (', reziser, ')') AS film, FROM filmovi;
ORDER BY naslov, reziser;

SELECT CONCAT(naslov, ' (', reziser, ')') AS film, god_izdavanja AS god FROM filmovi
ORDER BY naslov, reziser;

--selektovati sve filmove u obliku naslov (reziser) - godina izdanja. 
--Selektovane filmove soritrati rastuce prema godini izdanja.

SELECT CONCAT(naslov, ' (', reziser, ') - ', god_izdavanja) AS filmovi 
FROM filmovi 
...
--Funkcija MAX 
--funkcija MIN
SELECT MAX(ocena) AS max_ocena FROM filmovi;
SELECT MIN(ocena) AS min_ocena FROM filmovi;

--funkcija COUNT - broji redove

--odrediti broj filmova koji su izasli u 21. veku
...
--odrediti srednju ocenu svih filmova
SELECT AVG(ocena) AS srednja_ocena FROM filmovi;

--odrediti sumu ocena svih fimlova
SELECT SUM(ocena) AS ukupno
FROM filmovi;
